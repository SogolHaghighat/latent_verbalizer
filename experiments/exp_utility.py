...
def get_all_defined_classes_per_layer(data, layer_no, batch_size):
    ...
    for i in range(batch_size):
        try:
            my_dict = json.loads(
                data[i], object_pairs_hook=handle_duplicates
            )  # ast.literal_eval(data[f'layer_{layer_no}'][i])
...
def get_layer_class_dist(curr_layer_class_dist, final_list_classes):
    ...
    aggregated_percentages = {
        k: sum(d[k] for d in percentages) / len(curr_layer_class_dist)
        for k in percentages[0]
    }

    # Convert to DataFrame for easier plotting
    df = pd.DataFrame(
        list(aggregated_percentages.items()), columns=["Class", "Percentage"]
    )
    ...
    for cap in top1[f"layer_{layer_no}"]:
        ...
        for c in cap:
            if c in list(all_labels.keys()):
                ...
                adapted_cap.append(
                    "UNKNOWN"
                )  # as it is not clear what these exceptions are, they are replaced by UNKNOWN flag
